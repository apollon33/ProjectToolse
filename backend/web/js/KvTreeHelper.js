(function ($) {
    ...
    var $h, TreeView , $constant;
    $constant = {
       NODE_TYPE_DOCUMENT: 1,
       NODE_TYPE_ATTACHMENT: 3,
    ...
    TreeView.prototype = {
        ...
        renderForm: function (key, par, mesg, type) {
            var self = this, $detail = self.$detail, parent = par || '', msg = mesg || false, fileType = type || $constant.NODE_TYPE_DOCUMENT,
            ...
            $.ajax({
                ...
                success: function (data, textStatus, jqXHR) {
                    ...
                    if (data.nodeType == $constant.NODE_TYPE_ATTACHMENT) {
                        self.disable('create');
                        self.disable('createA');
                    }
                },
        ...
        create: function (type) {
            var type = type || $constant.NODE_TYPE_DOCUMENT , self = this, $nodeText = self.$tree.find('li .kv-node-detail.kv-focussed'), $n, key,
            ...
            if (type === $constant.NODE_TYPE_DOCUMENT) {
                self.renderForm(null, $node.data('key'), type);
            } else {
                self.renderForm(null, $node.data('key'), msg, type);
            }
        ...
        listen: function () {
            ...
            self.$tree.find('.kv-node-detail').each(function () {
                $(this).on('click', function () {
                    ...
                    if ($('.js-node-type').val() == $constant.NODE_TYPE_ATTACHMENT) {
                        self.disable('create');
                        self.disable('createA');
                    }
                });
            ...
            self.$toolbar.find('.kv-' + self.btns.createA).on('click', function () {
                self.create($constant.NODE_TYPE_ATTACHMENT);
            });
