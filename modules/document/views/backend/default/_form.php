...
<?php if (!$noNodesMessage): ?>
    ...
    if ($showIDAttribute) {
        ...
        $keyField = $form->field->field($node, $keyAttribute, $fieldOptions)->textInput($options);
    } else {
    ...
    <?php if ($iconsList == 'text' || $iconsList == 'none'): ?>
        <?php if ($showIDAttribute): ?>
            <div class="row">
                ...
                <div class="col-sm-8">
                    <?= $form->field($node, $nameAttribute, $fieldOptions)->textInput($inputOpts) ?>
                </div>
            </div>
        <?php else: ?>
            <?= $keyField ?>
        ...
        <?php if ($iconsList === 'text'): ?>
            <div class="row">
                <div class="col-sm-4">
                    <?= $form->field($node, $iconTypeAttribute, $fieldOptions)->dropdownList([
                        TreeView::ICON_CSS => 'CSS Suffix',
                        TreeView::ICON_RAW => 'Raw Markup',
                    ], $inputOpts) ?>
                </div>
                <div class="col-sm-8">
                    <?= $form->field($node, $iconAttribute, $fieldOptions)->textInput($inputOpts) ?>
                </div>
            </div>
        <?php endif; ?>
    <?php else: ?>
        <div class="row">
            <div class="col-sm-6">
                ...
                <?= $form->field($node, $nameAttribute, $fieldOptions)->textArea(['rows' => 3] + $inputOpts) ?>
            </div>
            <div class="col-sm-6">
                <?= /** @noinspection PhpUndefinedMethodInspection */
                $form->field($node, $iconAttribute, $fieldOptions)->multiselect($iconsList, [
