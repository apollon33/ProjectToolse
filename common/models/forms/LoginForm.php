...
class LoginForm extends Model
{
        ...
        if (!$this->hasErrors()) {
            ...
            if ($user) {
                ...
                if (!$user->getActiveUser()) {
                    $this->addError($attribute, Yii::t('app', 'Your account is locked. Contact the administrator!'));
                }
            } else {
                $this->addError($attribute, Yii::t('app', 'Incorrect email or password'));
            }
    ...
    {
        if ($this->validate()) {
            $this->getUserByEmail()->touch('last_login_at');
    ...
    {
        return $this->_user === null ? User::findByEmail($this->email) : $this->_user;
    }
}
