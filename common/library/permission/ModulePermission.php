...
class ModulePermission
{
     ...
     * @param array $nodesChain
     */
    protected function unRegisterNode(array $node)
    {
        foreach ($node as $item) {
            $permission = $this->authManager->getPermission($item);
            if ($permission !== null) {
     ...
     * @param array $nodesChain
     */
    protected function registerNode(array $node)
    {
        foreach ($node as $item) {
            $permission = $this->authManager->createPermission($item[1]);
            $permission->description = $item[0];
